...
- `lower_const(ConstValue_ptr value, RealType_ptr expected_type)` 的行为：
  ...
  * `ConstValueKind::ANYINT` 在 const-eval 完成后理论上不应再出现：若仍遇到该枚举，直接抛出 `TypeLoweringError("ConstValue ANYINT not concretized")` 要求语义阶段补全类型。其余整型常量按 `I32/ISIZE` 符号扩展、`U32/USIZE` 零扩展处理。
...
- `lower_const()` 覆盖：
  ...
  * 传入与预期类型不匹配时断言会抛出 `TypeLoweringError`，说明函数不会悄悄折叠或强行截断。
